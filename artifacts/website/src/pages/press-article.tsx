...
function isStoredAudio(url: string) {
  return url.startsWith("/media/") || url.startsWith("/api/storage") || /\.(mp3|m4a|wav|ogg|aac)$/i.test(url);
}

function isStoredVideo(url: string) {
  return url.startsWith("/media/") || url.startsWith("/api/storage") || /\.(mp4|webm|mov)$/i.test(url);
}

function isPdf(url: string) {
  return url.startsWith("/media/") || url.startsWith("/api/storage") || /\.pdf$/i.test(url);
}
        ...
        {isLoading ? (
          ...
          <>
            ...
            <article className="container max-w-3xl mx-auto px-4 md:px-8 py-12">
              {/* Media Section - Consistent across all types */}
              <section className="mb-12">
                ...
                {post.mediaUrl && post.type === "presentation" && (
                  <div className="rounded-xl bg-gradient-to-r from-muted to-muted/50 border border-border p-6">
                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-4 flex items-center gap-2">
                      <Headphones className="h-4 w-4" /> Audio Blog
                    </p>
